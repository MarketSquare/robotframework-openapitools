...
Test Get Ids From Url That Returns Single Resource
    ${url}=    Get Valid Url    path=/wagegroups/{wagegroup_id}    method=post
...
Test Get Ids From Url That Returns List Of Resources
    ...
    Get Valid Url    path=/employees/{employee_id}    method=get
    ${url}=    Get Valid Url    path=/employees    method=get
    ${ids}=    Get Ids From Url    url=${url}
